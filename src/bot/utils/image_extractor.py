...
def validate_file_path(
    ...
    """Validate an arbitrary file path from an MCP ``send_file_to_user`` call.

    Unlike :func:`validate_image_path`, this accepts ANY extension (documents,
    archives, etc.) so the agent can deliver generated files (.docx, .pdf, .md…)
    to Telegram. The file must be an existing regular file inside
    *approved_directory* and not exceed the Telegram size cap. Returns an
    :class:`ImageAttachment` (reused as a generic attachment) or ``None``.
    """
    import mimetypes
    ...
    try:
        ...
        if file_size == 0 or file_size > MAX_FILE_SIZE_BYTES:
            logger.debug("MCP file size out of range", path=str(resolved), size=file_size)
            return None
