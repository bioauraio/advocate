...
class ClaudeCodeBot:
    ...
    def _create_middleware_handler(self, middleware_func: Callable) -> Callable:
        ...
        async def middleware_wrapper(
            ...
            # Ignore Telegram service messages — pins, joins/leaves, title/photo
            # changes, etc. The bot must not react to them in any way (no auth
            # prompt, no Claude response). Normal messages never set these fields.
            ...
            if msg is not None and any(
                ...
                logger.debug(
                    "Skipping Telegram service message in middleware",
                    middleware=middleware_func.__name__,
                )
                raise ApplicationHandlerStop

            # Per-user context directory. If <approved_directory>/users/<user_id>/
            # exists, it becomes that user's working directory — giving them their
            # own CLAUDE.md (persona/context), own uploads/ and own style samples.
            # Users without such a folder keep the shared approved directory.
            # Set once per user; an explicit /cd by the user still wins.
            if update.effective_user and not context.user_data.get(
                "current_directory"
            ):
                user_dir = (
                    Path(self.settings.approved_directory)
                    / "users"
                    / str(update.effective_user.id)
                )
                if user_dir.is_dir():
                    context.user_data["current_directory"] = user_dir
                    logger.info(
                        "Per-user context directory applied",
                        user_id=update.effective_user.id,
