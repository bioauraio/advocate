...
async def auth_middleware(handler: Callable, event: Any, data: Dict[str, Any]) -> Any:
    ...
    # Chat (group) context — used for chat-based allow-list and logging
    chat = getattr(event, "effective_chat", None)
    chat_id = getattr(chat, "id", None)
    chat_type = getattr(chat, "type", None)
    settings = data.get("settings")

    # Group allow-list: any user writing in an approved chat is authorized
    # (e.g. the defense-team group), without being in allowed_users.
    allowed_chats = getattr(settings, "allowed_chats", None) if settings else None
    ...
    if authentication_successful:
        ...
        # Welcome message for new session — private chats only, to avoid
        # posting system chatter into group chats.
        if event.effective_message and chat_type == "private":
            await event.effective_message.reply_text(
        ...
        # Only notify in private chats. In groups/supergroups stay completely
        # silent — otherwise the bot spams "Authentication Required" on every
        # message from any non-whitelisted member of the chat.
        if event.effective_message and chat_type == "private":
            await event.effective_message.reply_text(
